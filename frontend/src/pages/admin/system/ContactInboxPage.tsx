...
export const ContactInboxPage: React.FC = () => {
  ...
  const archivedCount = useMemo(
    () => messagesList.filter((m) => m.status === 'archived').length,
    [messagesList]
  );

  const statusTabs = [
    { label: 'Active Inquiries', value: '' },
    { label: 'New Unread', value: 'new', count: unreadCount },
  ...
  const emptyState = (
    <div className="py-12 text-center font-sans space-y-3">
      ...
      <div className="space-y-1">
        <h3 className="text-sm font-bold text-text">No Correspondence Found</h3>
        <p className="text-xs text-textMuted max-w-sm mx-auto">
          {searchQuery || statusFilter
            ? 'No contact submissions match your current search or status filter.'
            : 'Your inbox is clear. Messages submitted through the public platform will appear here.'}
        </p>
      </div>
      {(searchQuery || statusFilter) && (
        <Button
