...
export function ForgotPasswordPage() {
  ...
  const onSubmit = async (data: ForgotPasswordFormData) => {
    try {
      ...
      toast.success('A password reset code has been dispatched to your email.');
      setTimeout(() => navigate('/reset-password', { state: { email: data.email } }), 1000);
    } catch (err: any) {
      const msg =
        err?.response?.data?.error?.message || 'Failed to dispatch reset code. Please try again.';
      toast.error(msg);
    }
  };
        ...
        <div className="relative rounded-2xl border border-stone-200/90 dark:border-zinc-800/80 bg-white/95 dark:bg-zinc-900/90 backdrop-blur-xl p-8 shadow-xl shadow-stone-300/40 dark:shadow-2xl dark:shadow-black/80 transition-colors duration-200">
          ...
          <div className="mb-8 text-center">
            ...
            <p className="mt-3 text-xs text-zinc-600 dark:text-zinc-400 leading-relaxed">
              Enter your registered email address to receive a secure password reset authorization code
            </p>
          </div>
          ...
          <div className="mt-6 pt-5 border-t border-stone-200/80 dark:border-zinc-800/80 flex items-center justify-center gap-1.5 text-[11px] text-zinc-500 dark:text-zinc-500">
            ...
            <span>End-to-end encrypted recovery protocol</span>
          </div>
        </div>
      </motion.div>
    </div>
