...
export function ArticleCard({ article, index = 0, hideTopicBadge = false }: ArticleCardProps) {
  ...
  return (
    <motion.article
      ...
      className="group flex flex-col justify-between rounded-2xl sm:rounded-3xl border border-border/80 bg-surface/90 backdrop-blur-xl overflow-hidden shadow-apple-card transition-all duration-300 hover:-translate-y-1.5 hover:shadow-apple-elevated hover:border-gold/50 active:scale-[0.98] h-full"
    >
      <Link to={`/articles/${article.slug}`} className="flex flex-col flex-1">
        {/* Visual Cover Banner — Minimized on mobile, rich on desktop */}
        <div className="relative h-28 sm:h-48 w-full overflow-hidden bg-stone-100 dark:bg-zinc-950 border-b border-border/60 shrink-0">
          {article.coverImageUrl ? (
            ...
            <div className="relative flex h-full w-full items-center justify-center bg-gradient-to-br from-[#b3854c]/15 via-stone-100 to-stone-50 dark:from-[#c9a84c]/15 dark:via-zinc-900 dark:to-zinc-950">
              ...
              <div className="h-8 w-8 sm:h-12 sm:w-12 rounded-xl sm:rounded-2xl bg-white/80 dark:bg-zinc-900/80 border border-gold/30 shadow-apple-sm flex items-center justify-center text-gold transition-transform duration-500 group-hover:scale-110">
          ...
          {/* Topic Badge — Hidden on mobile or when hideTopicBadge is true */}
          ...
          {/* Reading Time Badge — Minimized on mobile */}
          <div className="absolute top-2 right-2 sm:top-3 sm:right-3 z-10">
            <span className="inline-flex items-center gap-1 rounded-full border border-white/20 bg-black/60 text-zinc-200 backdrop-blur-md px-1.5 py-0.5 sm:px-2.5 sm:py-0.5 text-[9px] sm:text-[10px] font-medium shadow-apple-sm">
        ...
        {/* Card Body — Clean & minimized on mobile */}
        <div className="flex flex-col flex-1 justify-between p-2.5 sm:p-5 space-y-2 sm:space-y-3">
          <div className="space-y-1 sm:space-y-2">
            <h3 className="text-xs sm:text-lg font-bold text-zinc-900 dark:text-zinc-100 leading-snug group-hover:text-gold transition-colors line-clamp-2">
              {article.title}
            </h3>
          ...
          {/* Card Footer: Clean, essential byline & action */}
          <div className="pt-2 sm:pt-3 border-t border-stone-200/80 dark:border-zinc-800/80 flex items-center justify-between text-[10px] sm:text-xs text-zinc-500 dark:text-zinc-400">
            <div className="flex items-center space-x-1.5 sm:space-x-2 truncate">
              ...
              <span className="font-semibold text-zinc-800 dark:text-zinc-200 truncate max-w-[65px] sm:max-w-none text-[10px] sm:text-xs">
                {article.author?.name ? article.author.name.split(' ')[0] : 'Qindil'}
              </span>
            </div>

            <div className="flex items-center space-x-1.5 sm:space-x-3 text-[10px] sm:text-xs shrink-0">
              {/* Raw view count hidden on mobile */}
              ...
              <span className="hidden sm:inline text-zinc-400 dark:text-zinc-500 text-[11px]">
                {formattedDate}
              </span>
              ...
              <div className="flex items-center space-x-0.5 text-gold font-bold group-hover:translate-x-0.5 transition-transform">
                ...
                <Icon name="ArrowRight" size={11} className="sm:w-3 sm:h-3" />
              </div>
            </div>
          </div>
        </div>
